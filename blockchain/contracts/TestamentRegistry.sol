...
contract TestamentRegistry {
  ...
  event NotaryRegistered(address indexed notary, string name, uint256 timestamp);
  event NotaryPinged(address indexed notary, uint256 timestamp);
  event NotaryDeactivated(address indexed notary, address indexed deactivatedBy, uint256 timestamp);
  event NotaryRevived(address indexed notary, address indexed revivedBy, uint256 timestamp);
  ...
  // ============ NOTARY FUNCTIONS ============
  function notaryPing() external onlyNotary {
    notaries[msg.sender].lastActive = block.timestamp;
  }
